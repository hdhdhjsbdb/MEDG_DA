...
import config
from MyModel import FeatureEncoder
...
class MCD_solver(object):
    def __init__(self, in_channels: int, feat_dim: int, num_classes: int, num_k: int):
        ...
        self.C1 = LabelClassifier(feat_dim=feat_dim, num_classes=num_classes)
        self.C2 = LabelClassifier(feat_dim=feat_dim, num_classes=num_classes)
    ...
    def train(self,epoch, source_loader, target_loader, device):
        ...
        for _ in range(min(len(source_loader),len(target_loader))):
            ...
            cls_loss1 = F.cross_entropy(output_s1, y_s) #源域保证分类能力
            cls_loss2 = F.cross_entropy(output_s2, y_s)
            cls_loss = cls_loss1 + cls_loss2

            f_t = self.F(x_t)  #目标域最大化差异
        ...
        for _ in range(self.num_k):
            ...
            for x_t, _, _ in iter_target:
                ...
                if _ == 0:  # 第一个epoch的num_k循环中只跑少量batch
                    break

        return cls_loss.item() if 'cls_loss' in locals() else 0.0
...
def train_MCD(eopchs,solver,source_loader,target_loader,val_loader,device):
    ...
    for epoch in range(1,epochs+1):
        ...
        print(f"Epoch {epoch}| Source Loss: {cls_loss:.4f}| tgtval_loss: {tgtval_loss:.4f}| tgtval_acc: {tgtval_acc:.4f}")
        ...
        if val_acc > best_val_acc:
            ...
            print(f"  >>> New best: Epoch {epoch}, Val Acc {val_acc*100:.2f}%")
...
if __name__ == "__main__":
    ...
    num_classes = config.MCD_num_class

    log_msg("=" * 80)
    log_msg(f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}, 任务: {config.TASK},seed: {args.seed}")
    log_msg("=" * 80)
    log_msg(f"参数:")
    log_msg(f"  num_classes: {num_classes},batch_size: {config.MCD_batch_size}, lr: {config.MCD_lr}, epochs: {config.epochs}")

    src_loader = DataLoader(
        source_ds, batch_size=config.MCD_batch_size, shuffle=False,
        num_workers=8, pin_memory=True, drop_last=False
    )

    tgt_loader = DataLoader(
        target_ds, batch_size=config.MCD_batch_size, shuffle=False,
        num_workers=8, pin_memory=True, drop_last=False
    )
    ...
    solver = train_MCD(
        epoch = config.MCDepochs,
        solver = solver,
        source_loader = src_loader,
        target_oader = tgt_loader,
    ...
    avg_loss, accuracy, macro_f1, weighted_f1 = eval_cls(solver, test_loader, device="cuda")
